...
# Kernel for Intrinsic Model of Coregionalization (IMC)
# Note: Adapted from GPflow implementation of LinearCoregionalization and SharedIndependent
class IntrinsicCoregionalization(gpflow.kernels.MultioutputKernel):
    def __init__(self, kernel, W):
        super().__init__()
        self.kernel = kernel
        self.W = Parameter(W) # [P, L]
        self.latent_dim = self.W.shape[-1] # L
    ...
    def K(self, X, X2=None, full_output_cov=True):
        K_i = self.kernel.K(X, X2) # [N, N2]
        #Ks = tf.stack([K for _ in range(self.num_latent_gps)], axis=0) # [L, N, N2]
        Ks = tf.tile(K_i[None,...], [self.num_latent_gps,1,1])
        KsW = Ks[None,:,:,:] * self.W[:,:,None,None] # [P, L, N, N2]
        
        if full_output_cov:
            WKsW = tf.tensordot(self.W, KsW, [[1], [1]]) # [P, P, N, N2]
            return tf.transpose(WKsW, [2,0,3,1]) # [N, P, N2, P]
        else:
            return tf.reduce_sum(self.W[:,:,None,None] * KsW, [1]) # [P, N, N2]
        
    def K_diag(self, X, full_output_cov=True):
        K_diag_i = self.kernel.K_diag(X)
        Ks_diag = tf.tile(K_diag_i[..., None], [1, self.num_latent_gps])
        #Ks_diag = tf.stack([K_diag for _ in range(self.num_latent_gps)], axis=1) # [N, L]
        
        if full_output_cov:
            Wt = tf.transpose(self.W) # [L, P]
            return tf.reduce_sum(
                Ks_diag[:,:,None,None] * Wt[None,:,:,None] * Wt[None,:,None,:], axis=1
            ) # [N, P, P]
        else:
            return tf.linalg.matmul(Ks_diag, self.W ** 2.0, transpose_b=True)

# Multiple dispatch Kuu for IMC kernel
@Kuu.register(SharedIndependentInducingVariables, IntrinsicCoregionalization)
def _Kuu(
    inducing_variable: SharedIndependentInducingVariables,
    kernel: IntrinsicCoregionalization,
    *,
    jitter=0.,
):
    Kmm_i = Kuu(inducing_variable.inducing_variable, kernel.kernel) # [M,M]
    Kmm = tf.tile(Kmm_i[None,...], [kernel.num_latent_gps,1,1]) # [L,M,M]
    jittermat = tf.eye(inducing_variable.num_inducing, dtype=Kmm.dtype)[None,:,:] * jitter # [1,M,M]
    
    return Kmm + jittermat # [L,M,M]

# Multiple dispatch Kuf for IMC kernel
@Kuf.register(SharedIndependentInducingVariables, IntrinsicCoregionalization, object)
def _Kuf(
    inducing_variable: SharedIndependentInducingVariables,
    kernel: IntrinsicCoregionalization,
    Xnew: tf.Tensor,
):
    Kmn_i = Kuf(inducing_variable.inducing_variable, kernel.kernel, Xnew) # [M,N]
    Kmn = tf.tile(Kmn_i[None,...], [kernel.num_latent_gps,1,1]) # [L,M,N]
    
    return Kmn

# Multiple dispatch conditional for IMC kernel
@conditional.register(
    object,
    SharedIndependentInducingVariables,
    IntrinsicCoregionalization,
    object,
)
def intrinsic_coregionalization_conditional(
    Xnew,
    inducing_variable,
    kernel,
    f, # This is q_mu
    *,
    ...
    Kmms = Kuu(inducing_variable, kernel, jitter=default_jitter()) # [L,M,M]
    Kmns = Kuf(inducing_variable, kernel, Xnew) # [L,M,N]
    
    if full_cov:
        Knn = kernel.kernel.K(Xnew) # [N,N]
        Knns = tf.tile(Knn[None,...], [kernel.num_latent_gps,1,1]) # [L,N,N]
    else:
        Knn = kernel.kernel.K_diag(Xnew) # [N,]
        Knns = tf.tile(Knn[None,...], [kernel.num_latent_gps,1]) # [L,N]
        
    fs = tf.transpose(f)[:,:,None] # [L,M,1] # Note: NOT function values; variational means
    ...
    if q_sqrt is not None:
        ...
        # [L,M,M], [L,M,N], [L,N,N], [L,M,1], [L,1,M,M]
        ...
        #base_conditional_args_to_map = (Kmms[0,...], Kmns[0,...], Knns[0,...], fs[0,...])
        
        def single_gp_conditional(t):
            Kmm, Kmn, Knn, f = t

            return base_conditional(Kmn, Kmm, Knn, f, full_cov=full_cov, q_sqrt=None, white=white)
    
    # Map over axis=0 (latent GPs)
    rmu, rvar = tf.map_fn(
        single_gp_conditional, base_conditional_args_to_map, (default_float(), default_float())
    )

    fmu = rollaxis_left(tf.squeeze(rmu, axis=-1), 1) # [N,L]
    
    '''
    if full_cov:
        # rvar: [L, 1, N, N]
        fvar = tf.squeeze(rvar, axis=-3)  # [..., 0, :, :]  # [L, N, N]
    #if full_cov and full_output_cov:
    #    fvar = tf.squeeze(rvar, axis=-3) # 

    else:
        # rvar: [L, N, 1]
        fvar = rollaxis_left(tf.squeeze(rvar, axis=-1), 1)  # [N, L]
    
    #return mix_latent_gp(kernel.W, fmu, expand_independent_outputs(fvar, full_cov, full_output_cov), 
    #                     full_cov, full_output_cov)
    return mix_latent_gp(kernel.W, fmu, fvar, full_cov, full_output_cov)
    '''

    if full_cov and full_output_cov:
        fvar = tf.squeeze(rvar, axis=-3) # [L,N,N]

        return mix_latent_gp(kernel.W, fmu, fvar, True, True) # [N,P,N,P]

    elif full_cov and not full_output_cov:
        fvar = tf.squeeze(rvar, axis=-3) # [L,N,N]

        return mix_latent_gp(kernel.W, fmu, fvar, True, False) # [P,N,N]

    elif not full_cov and full_output_cov:
        fvar = rollaxis_left(tf.squeeze(rvar, axis=-1), 1) # [N,L]

        return mix_latent_gp(kernel.W, fmu, fvar, False, True) # [N,P,P]

    elif not full_cov and not full_output_cov:
        fvar = rollaxis_left(tf.squeeze(rvar, axis=-1), 1) # [N,L]

        return mix_latent_gp(kernel.W, fmu, fvar, False, False) # [N,P]
...
def get_imc_svgp(
    ...
    """Returns a GPflow SVGP model with IMC kernel."""

    M = imc_params['M'] # Number of inducing points
    T = train_times.shape[0] # Number of time points
    P = imc_params['P'] # Number of outputs
    L = imc_params['L'] # Number of latent GPs
    kernel_type = imc_params['kernel_type'] # Type of kernel function
    ind_init_mode = imc_params['ind_init_mode']

    # Define IMC kernel
    if kernel_type == 'SE':
        _kernel = SquaredExponential()
    ...
    kernel = IntrinsicCoregionalization(_kernel, W=W)
...
def run_adam_natgrad(
    ...
    check_budget=False,
    start_time=None,
    time_budget=None # Should be a relativedelta object
):
...
def train_imc_svgp(
    ...
    # Model
    imc_params = dict(M=M, P=P, L=train_params['L'], kernel_type=kernel_type, ind_init_mode=ind_init_mode)
    model = get_imc_svgp(imc_params, train_times, fix_ind=fix_ind, random_init=random_init, seed=seed,
...
def imc_svgp_predict(model, data):
    """Performs approximate posterior inference with IMC on given batch of data."""
    ...
    _, F_cov = model.predict_f(times, full_cov=False, full_output_cov=True) # [T,P,P]
...
def compute_metrics_imc_svgp(model, data, preds):
