...
def train_LMC_SVGP(data, data_id, use_natgrad=True):
    ...
    int_preds = lmc_svgp_predict(model, (data['test_int']['times'], data['test_int']['Y']))
...
def run_experiment(data_id, use_natgrad=True):
    """Trains LMC on real data."""

    dataset = ID_TO_DATASET[data_id]
    print(f'Running LMC on {dataset.upper()} (NGD={use_natgrad}).')
    ...
    # Train model
    elbos, preds, maes, nlpds = train_LMC_SVGP(data, data_id, use_natgrad=use_natgrad)
    ...
    # Don't save predictions if using COVID data
    if data_id == 0:
    ...
    pkl_str = f'lmc_svgp_results_{dataset}'
    ...
    print(f'[{dataset.upper()}] LMC results saved: {pkl_path}.\n')
