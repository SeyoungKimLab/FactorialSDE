...
def train_IMC_SVGP(data, ind_id, split_id, use_natgrad=True):
...
def run_experiment(ind_id, split_id, use_natgrad=True):
    """Trains IMC on large simulation data with different number of inducing points."""

    M = ID_TO_M[ind_id-1]
    print(f'Running IMC on fold {split_id} (M={M}, NGD={use_natgrad}).')
    ...
    # Train model
    elbos, preds, maes, nlpds = train_IMC_SVGP(data, ind_id, split_id, use_natgrad=use_natgrad)
    ...
    pkl_str = f'imc_svgp_results_{split_id}_{M}'
    ...
    print(f'[Runtime] IMC results saved: {pkl_path}.\n')
