...
def mean_std(metrics):
    ...
    metrics = dict(int=int_metrics, ext=ext_metrics)
...
def save_results(model_type, results, use_natgrad, split_id=-1):
    if model_type == 'fsde':
        pkl_id = 'fsde_results_CV' if split_id == -1 else f'fsde_results_{split_id}'
        pkl_id += '_natgrad' if use_natgrad else '_adam'
        
    elif model_type == 'svi':
        pkl_id = 'svi_results_CV' if split_id == -1 else f'svi_results_{split_id}'
        pkl_id += '_natgrad' if use_natgrad else '_adam'
...
def train_FSDE(data, split_id, use_natgrad=True):
    ...
    exp_metrics = model.fit(n_steps=N_STEPS, lr=lr, lr_steps=500, use_natgrad=use_natgrad)
    elbos = exp_metrics[0]
...
def run_experiments(model_type, use_natgrad=True, split_id=-1):
    ...
    if split_id == -1:
        ...
        elbos, preds, maes, nlpds = train_f(data, split_id, use_natgrad=use_natgrad)
